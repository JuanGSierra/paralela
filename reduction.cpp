 ...
 * 
 * comando para compilar: g++ reduction.cpp -o testoutput -pthread -std=c++11 `pkg-config --cflags --libs opencv`
 * 
...
int main(int argc, char *argv[])
{
  ...
  // se genera el flujo de trabajo a los hilos
  for (i = 0; i < THREADS; i++)
  {
    threadId[i] = i;
    pthread_create(&thread[i], NULL, resample, &threadId[i]);
  }

  // se esperan que terminen todos los hilos
  for (i = 0; i < THREADS; i++)
  {
